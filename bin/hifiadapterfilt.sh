...
# This tool was developed by Sheina B. Sim et al. This code was taken from their
# tool at: https://github.com/sheinasim/HiFiAdapterFilt.
# Citation is available there.
...
printf "Prefix: %s\nPath: %s\nStr: %s\n\n" ${reads_pref} ${read_path} ${read_path_str}

## Create out directory if necessary
printf "[3. Creating output directory]\n"
if [ ! -d ${outdir} ]
then 
	mkdir ${outdir}
fi
